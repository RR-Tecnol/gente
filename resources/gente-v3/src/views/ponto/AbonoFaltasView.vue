  ...
  <div class="abono-page">
    ...
    <div v-if="tabAtiva === 'Justificar Falta'" class="tab-content" :class="{ loaded }">
      <div class="form-panel">
        ...
        <div class="form-group">
          <label>Comprovante <span class="field-hint-inline">(PDF, JPG ou PNG)</span></label>
          <div
            class="upload-zone"
            :class="{ 'drag-over': isDragging, 'has-file': !!arquivoSelecionado }"
          ...
          >
            ...
            <template v-if="!arquivoSelecionado">
              ...
              <p class="upload-label">Arraste o arquivo aqui ou <span class="upload-link">clique para selecionar</span></p>
              <p class="upload-hint">Máx. 5 MB · PDF, JPG, PNG</p>
            </template>
            <template v-else>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#6366f1" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
              <p class="upload-label" style="color:#4f46e5;font-weight:700">{{ arquivoSelecionado.name }}</p>
              <button class="upload-remove" type="button" @click.stop="arquivoSelecionado = null">✕ Remover</button>
            </template>
          </div>
        </div>
    ...
    <div v-if="tabAtiva === 'Histórico de Abonos'" class="tab-content" :class="{ loaded }">
      ...
      <div v-else class="abonos-grid">
        ...
        >
          ...
          <!-- Comprovante -->
          <div v-if="a.comprovante_url" class="card-comprovante">
            <a :href="a.comprovante_url" target="_blank" rel="noopener" class="comprovante-link">
...
const enviar = async () => {
  ...
  payload.append('ABONO_FALTA_DATA', form.ABONO_FALTA_DATA)
  payload.append('ABONO_FALTA_JUSTIFICATIVA', form.tipo
    ? `[${tipoLabelStr(form.tipo)}] ${form.ABONO_FALTA_JUSTIFICATIVA}`
    : form.ABONO_FALTA_JUSTIFICATIVA)
  payload.append('justificativa', form.ABONO_FALTA_JUSTIFICATIVA)
  if (form.tipo) payload.append('tipo', form.tipo)
  if (arquivoSelecionado.value) payload.append('comprovante', arquivoSelecionado.value)
...
/* UPLOAD ZONE */
.field-hint-inline { font-size: 10px; color: #94a3b8; font-weight: 400; text-transform: none; letter-spacing: 0; }
.upload-zone {
  border: 2px dashed #e2e8f0; border-radius: 14px; padding: 22px 16px;
  display: flex; flex-direction: column; align-items: center; gap: 8px;
  cursor: pointer; background: #f8fafc; transition: all 0.18s; text-align: center;
...
.upload-zone.has-file   { border-color: #6366f1; border-style: solid; background: #f5f3ff; }
.upload-label { font-size: 13px; color: #64748b; margin: 0; }
.upload-link  { color: #6366f1; font-weight: 700; text-decoration: underline; }
.upload-hint  { font-size: 11px; color: #94a3b8; margin: 0; }
.upload-remove { background: none; border: 1px solid #c7d2fe; border-radius: 8px; color: #6366f1; font-size: 11px; font-weight: 700; padding: 3px 10px; cursor: pointer; font-family: inherit; transition: all 0.15s; }
.upload-remove:hover { background: #eef2ff; }

/* COMPROVANTE LINK */
.card-comprovante { display: flex; }
.comprovante-link {
  display: inline-flex; align-items: center; gap: 5px;
  padding: 4px 10px; border-radius: 8px; font-size: 12px; font-weight: 700;
  color: #4f46e5; background: #f5f3ff; border: 1px solid #ddd6fe; text-decoration: none;
